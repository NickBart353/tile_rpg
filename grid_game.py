...
while running:
    ...
    while game_running:
        ...
        for i in range(0, COLUMNS):
            for j in range(0, ROWS):
                if positions[j][i].image:
                    screen.blit(positions[j][i].image, ((i-x)*SIZE, (j-y)*SIZE))
                if x+13 == i and y+10 == j:
                    match positions[j][i].exit_id:
                        case 0:
                            x = 1-13
                            y = 13-10
        ...
        while movement_counter > 0:
            ...
            match direction:
                case "UP":
                    if positions[y - 1+10][x+13]:
                        if not positions[y - 1+10][x+13].collidable:
                            y -= 1
                        else:
                            bump_sound.play()
                case "DOWN":
                    if not positions[y + 1 + 10][x + 13].collidable:
                        ...
                        bump_sound.play()
                case "LEFT":
                    if not positions[y + 10][x - 1 + 13].collidable:
                        x -= 1
                    else:
                        bump_sound.play()
                case "RIGHT":
